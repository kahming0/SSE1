...
for exec in $experiments ; do
    ...
    then
        ...
        sysbench /usr/share/sysbench/oltp_read_write.lua --db_driver=mysql --threads=1 --mysql-host=127.0.0.1 --mysql-user=sbtest --mysql-password=password --mysql-port=3306 --tables=$NUM_TABLES --table-size=$TSIZE --events=$NUM_OF_TRANSACTIONS run        
    ...
    # Now we do the postgres script
        ...
        sysbench /usr/share/sysbench/oltp_read_write.lua --db-driver=pgsql --threads=1 --pgsql-host=127.0.0.1 --pgsql-user=sbtest --pgsql-password=password --pgsql-port=5432 --tables=$NUM_TABLES --table-size=$TSIZE --events=$NUM_OF_TRANSACTIONS run
